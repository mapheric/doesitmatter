...
public class Parser2
{
    ...
    private static ArrayList<Integer> replace = new ArrayList<Integer>();
...
//        	}
        ...
        if(curToken.getToken().equals("{"))
        {
            ...
            if(isValid)
            {
                ...
                if(isValid)
                {
                    ...
                    if(curToken.getToken().equals("}"))
                    {
                    	if(copy.get(current).equals("}") && !operator.get(operator.size() - 1).equals("END")) {
                        	add("END", "BLOCK", "", "");
                        	replace.add(operator.size());	//maybe need to check if we're not in a nested block?
                        }
                        removeToken();
                        return;
                    }
    ...
    private void statement()
    {
          ...
          )
        {
        	...
        	if(!curToken.getToken().equals("(") && !curToken.getToken().equals(";")){
        		...
        		if((curToken.getTokenType().equals("INT") || curToken.getTokenType().equals("FLOAT")) && !nextToken().equals("[")){
        			...
        			System.out.println("__________Something");
        		}else if((curToken.getTokenType().equals("ID") || curToken.getTokenType().equals("INT") || curToken.getTokenType().equals("INT")) && nextToken().equals("=")){
        			//System.out.println("--------> Operand: " + copy.get(current + 3) + " ID: " + copy.get(current));
        			assignToThis = curToken.getToken();
        			if(copy.get(current + 3).equals(";")) {
        				//System.out.println("------> OP_1: " + copy.get(current + 3));
        				add("ASSIGN", copy.get(current + 2), "", assignToThis);
        			}else{  
        				//System.out.println("--------> Operand: " + copy.get(current + 3) + " ID: " + copy.get(current));
        ...
        {
        	if(!operator.get(operator.size() - 1).equals("BLOCK")) {
        		add("BLOCK", "", "", "");
        	}
            compoundStmt();
            return;
        ...
        {
            //System.out.println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~Current Token: " + curToken.getToken());
            selectionStmt();
            //System.out.println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~After if - token: " + curToken.getToken());
            return;
        }
        ...
        else if(curToken.getToken().equals("while"))
        {
            int place = current;
            boolean tester = true;
        	while(!copy.get(place).equals("}") && tester == true){
        		if(copy.get(place).equals("<=")) {
        			add("BRLTEQ", "","", "");
        			find.add(operator.size()-1);
        			tester = false;
        		}else if(copy.get(place).equals("<")) {
        			add("BRLT", "","", "");
        			find.add(operator.size()-1);
        			tester = false;
        		}else if(copy.get(place).equals(">")) {
            ...
            iterationStmt();
            //System.out.println(copy.get(current));
            if(copy.get(current).equals("}") && !operator.get(operator.size() - 1).equals("END")) {
            	add("END", "BLOCK", "", "");
            	replace.add(operator.size());
            }
            /***TODO: insert after block statment here; 
             * BackPatch: 
             * 		make two arrays
             * 		store start point in one, end point in the other - using same index (global)
             * 		iterate through them at the end, and replace the start index with the global value
             * **/
            return;
        }
...
//		ListIterator<String> o_res = result.listIterator();
    ...
    public static void print(){
    	...
    	for(int d = 0; d<find.size()-1; d++) {
    		if(d>replace.size()-1) {
    			result.set(find.get(d), Integer.toString((int)Math.floor((Math.random() * result.size()-1) + find.get(b))));
    		}else {
    			result.set(find.get(d), Integer.toString(replace.get(d)));
    			//result.set(replace.get(d), Integer.toString(find.get(d)));
    		}
		}
    	System.out.println();
...
//		}//end of while loop
		...
		while(o_rand_1.hasNext()) {
			...
			if((or.startsWith("BR") && res.equals(""))){
				for(int c = jjj; c<operator.size()-1; c++) {
					if(operand1.get(c).equals("BLOCK")) {
						res = Integer.toString(c);
					}
				}
			}			
			
			// || Integer.parseInt(res) <= jjj
    ...
    public static String parseExpression(String id, int place, String stopHere){			//parses expressions
    	int place_2 = place;
    	while(!copy.get(place_2).equals("stopHere")) {
    		//System.out.printf("%-5s %-15s %-15s %n",place_2 + ".", copy.get(place_2), copyTokenType.get(place_2));
    		place_2++;
    	}
    	String aTempVar = "";
    	if(id.equals(";")){
    		return "";	//error code
    	}else if((copy.get(place + 1).equals(";") && current == place) || (copy.get(place + 1).equals(")") && copy.get(place + 2).equals(";"))){	//if it's the end, return sum of balance  
    		//System.out.println("111111111111111111");
    		return id;
    	}else if(copy.get(place + 1).equals("stopHere") || copy.get(place).equals("stopHere")){						//if it's the end, return sum of balance
    		//System.out.println("***********************. Current token: " + copy.get(place) + " at position: " + place + ". Peek: " + tempVars.peek());
    		return parseAdd();
    	}else if(copy.get(place + 1).equals("(")){						//if it's a function  
    		return parseFunction(id,place);
    	}else if(copy.get(place + 1).equals(")")){			//(id.equals(copy.get(place + 1))){			//if it's the end of a bracketed computation				
    		return parseExpression(copy.get(place + 2), place + 2);
    	}else if(copy.get(place + 1).equals("[")){
    		return parseArr(id, place);
    	}else if(copyTokenType.get(place+1).equals("MINUS") || copyTokenType.get(place+1).equals("PLUS")){
    		//System.out.println("Heres what's been pushed: " + id + ", " + copy.get(place + 1));
    		tempVars.push(id);
    		tempVars.push(copy.get(place+1));
    		if(copyTokenType.get(place + 2).equals("ID") || copyTokenType.get(place + 2).equals("INT") || copyTokenType.get(place + 2).equals("FLOAT")) {
    			tempVars.push(copy.get(place+2));
    		}
    		return parseExpression(copy.get(place + 2), place + 2);
    		...
    		//System.out.println("~~~~~~~~~~~~~~~~~ In Return. id = " + id + ". id + 1: " + copyTokenType.get(place+1));
    		return id;
    	}
    }
    ...
    public static String parseExpression(String id, int place){			//parses expressions
    	...
    	if(id.equals(";")){
    		...
    		//System.out.println("111111111111111111");
    		return id;
    	}else if(copy.get(place + 1).equals(";") || copy.get(place).equals(";")){						//if it's the end, return sum of balance
    		//System.out.println("***********************. Current token: " + copy.get(place) + " at position: " + place + ". Peek: " + tempVars.peek());
    		return parseAdd();
    	}else if(copy.get(place + 1).equals("(")){						//if it's a function  
    		return parseFunction(id,place);
...
//    	}
    ...
    public static String parseAdd(){					//parses the addition/subtraction part of an expression
    	...
    	while(!curr.equals("$") && !curr.equals(null)){
    		//System.out.println("-------------------- Current = " + curr + ". id = " + id + ". ");
    		if(curr.equals("+")){	
    			bTempVars = getTempVar();
    			curr = tempVars.pop();
    			add("ADD", id, curr, bTempVars);  
    			//System.out.println("@@@ In ADD. Current = " + curr + ". id = " + id + ". ");
    		}else if(curr.equals("-")){
    			bTempVars = getTempVar();
    			curr = tempVars.pop();
    			add("SUB", id, curr, bTempVars);
    			//System.out.println("@@@ In SUB. Current = " + curr + ". id = " + id + ". ");
    		}else{
    			id = curr;
    			curr = tempVars.pop();
    			//System.out.println("---- Current = " + curr + ". id = " + id + ". ");
